...
export default function SiteEnhancements() {
  useEffect(() => {
    ...
    try {
      brandSrc = createWebpObjectUrl(BRAND_CHUNKS);
      restoreAbout = replaceAboutVisual(brandSrc);
    } catch (error) {
      console.error("Could not prepare MO Vida brand visual", error);
    }
    ...
    return () => {
      restoreAbout();
      if (brandSrc) URL.revokeObjectURL(brandSrc);
